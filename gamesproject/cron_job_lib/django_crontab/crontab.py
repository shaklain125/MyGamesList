...
class Crontab(object):
    ...
    def add_jobs(self):
        ...
        for job in self.settings.CRONJOBS:
            ...
            o_l = list(job)
            prefix_cmd = ''
            if l[1].startswith('prefix_cmd='):
                prefix_cmd = l[1][l[1].rfind('prefix_cmd=')+len('prefix_cmd='):]
                l.pop(1)
                job = tuple(l)
            ...
            self.crontab_lines.append(self.settings.CRONTAB_LINE_PATTERN % {
                ...
                'command': ' '.join(filter(None, [
                    ...
                    prefix_cmd,
                    '"{0}"'.format(self.settings.PYTHON_EXECUTABLE),
                    '"{0}"'.format(self.settings.DJANGO_MANAGE_PATH),
                    'crontab', 'run',
                    self.__hash_job(job),
